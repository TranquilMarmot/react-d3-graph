...
function snapPointToGrid(x, y, gridConfig) {
  var gridWidth = gridConfig.gridWidth,
    gridHeight = gridConfig.gridHeight,
    innerGridXDivisions = gridConfig.innerGridXDivisions,
    innerGridYDivisions = gridConfig.innerGridYDivisions; // have to find _actual_ width of each grid cell
  // we use `|| 1` in case it is set to undefined or 0

  var dividedGridWidth = gridWidth / (innerGridXDivisions || 1);
  var dividedGridHeight = gridHeight / (innerGridYDivisions || 1); // snap to x
