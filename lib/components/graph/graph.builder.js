...
function buildLinkProps(link, nodes, links, config, linkCallbacks, highlightedNode, highlightedLink, scaleFactor) {
  ...
  var strokeWidth = (link.strokeWidth || config.link.strokeWidth) * (1 / scaleFactor);
  ...
  var markerId = config.directed ? (0, _marker.getMarkerId)(highlight, scaleFactor, config) : null;
  var t = 1 / scaleFactor;
  var fontSize = null,
    fontColor = null,
    fontWeight = null,
...
function buildNodeProps(node, config) {
  ...
  var scaleFactor = arguments.length > 5 ? arguments[5] : undefined;
  var highlight =
    node.highlighted ||
    node.id === (highlightedLink && highlightedLink.source) ||
    node.id === (highlightedLink && highlightedLink.target);
